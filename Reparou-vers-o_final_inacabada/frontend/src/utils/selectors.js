...
export const selectClienteById = (state, id) =>
  state.data.clientes.find((item) => item.id === id);

export const selectLojistaById = (state, id) =>
  state.data.lojistas.find((item) => item.id === id);

export const selectLojaById = (state, id) =>
  state.data.lojas.find((item) => item.id === id);

export const selectReviewsByLoja = (state, lojaId) =>
  state.data.reviews
    .filter((item) => item.lojaId === lojaId)
    .sort((a, b) => new Date(b.dataIso) - new Date(a.dataIso));

export const selectChatsByCliente = (state, clienteId) =>
  (state.chat.chats || [])
