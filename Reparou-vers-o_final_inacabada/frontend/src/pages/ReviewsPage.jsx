...
export default function ReviewsPage() {
  ...
  const isGuest = role === 'guest' || user?.id === 'guest' || user?.role === 'guest';
  ...
  return (
    <Layout bottom={isGuest ? null : <BottomNavCliente />}>
      <div className="profile-header2 mb-3">
        ...
        <div className="text-center">
          <img src={loja.imagem} className="shop-img mb-2" alt="" />

          <h4>{loja.titulo}</h4>

          <div>{loja.enderecoLinhas.join(" • ")}</div>
        ...
        <div className="d-flex justify-content-center gap-2 loja-tabs mt-3">
          <Link className="loja-tab" to={`/lojas/${id}`}>
            Serviços
          </Link>
      ...
      <div className="card p-3 mb-5">
        <div className="d-flex flex-wrap gap-2 mb-3">
          {[...new Set(reviews.flatMap((r) => r.tags || []))].map(
            (tag) => (
              <span
                key={tag}
                    ...
                    <div className="my-1">
                      {(avaliacao.tags || []).map(
                        (tag) => (
                          <span
                            key={tag}
                            className="review-service-chip me-1"
                          >
                            {tag}
                          </span>
                        )
