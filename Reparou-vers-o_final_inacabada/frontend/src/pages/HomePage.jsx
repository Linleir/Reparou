...
export default function HomePage() {
  ...
  const cliente = useSelector((state) =>
    isGuest ? null : state.data.clientes.find((item) => item.id === user?.id)
  );
  ...
  const lojasExibidas =
    pesquisa.trim() || tagsSelecionadas.length ? lojasFiltradas : lojas;
          ...
          lojasExibidas.map((loja) => (
            <StoreCard
              ...
              onToggleFavorite={
                isGuest
                  ? null
                  : (lojaId) => dispatch(toggleFavorite({ clienteId: user.id, lojaId }))
              }
            />
          ))
        ) : (
