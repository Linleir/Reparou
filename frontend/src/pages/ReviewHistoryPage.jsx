...
import { deleteReview } from '../features/dataSlice';
    ...
    (state.data.reviews || []).filter(
      (item) =>
        ...
        (state.chat.chats || []).some(
          (chat) =>
            String(chat.id) === String(item.chatId) &&
            String(chat.clienteId) === String(user?.id)
        )
    )
  );
        ...
        {reviews.length ? (
          reviews.map((avaliacao) => {
            const loja = lojas.find(
              (l) => String(l.id) === String(avaliacao.lojaId)
            );
