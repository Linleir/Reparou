    ...
    <Layout bottom={<BottomNavCliente />}>
      <div className="profile-header2 mb-3">
        <div className="d-flex justify-content-between align-items-start">
          <Link to="/inicio" className="text-white text-decoration-none">←</Link>
        ...
        <div className="text-center">
          ...
          <h4>{loja.titulo}</h4>
          <div>{loja.enderecoLinhas[0]}<br />{loja.enderecoLinhas[1]}</div>
          <div>{loja.telefones.join(' • ')}</div>
          <div className="mt-2">★ {formatRating(loja.media)} ({loja.quantidadeAvaliacoes})</div>
        </div>

        <div className="d-flex justify-content-center gap-2 loja-tabs mt-3">
          <span className="loja-tab loja-tab-active">Serviços</span>
          <Link className="loja-tab" to={`/lojas/${id}/avaliacoes`}>Avaliações</Link>
          <Link className="loja-tab" to={`/lojas/${id}/denuncia`}>Denunciar</Link>
        </div>
      </div>
            ...
            >
              <div className="d-flex justify-content-between align-items-center gap-3">
                <div>
                  ...
                  <div className="small text-muted">{servico.preco}</div>
                  <div className="small text-muted">{servico.tag}</div>
                </div>

                <div className="text-end">
                  <div className="text-warning fw-bold">{formatRating(servico.nota)} ★</div>
                  <small className="text-muted">({servico.quantidadeAvaliacoes})</small>
                </div>
              </div>
