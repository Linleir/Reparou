...
export const selectChatsByLojista = (state, lojistaId) => {
  const lojasIds = state.data.lojas.filter((loja) => loja.lojistaId === lojistaId).map((loja) => loja.id);
  return state.data.chats.filter((chat) => lojasIds.includes(chat.lojaId)).sort((a,b)=>new Date(b.atualizadoEm)-new Date(a.atualizadoEm));
};
